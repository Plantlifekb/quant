...
Purpose
- Classify each trading day into a market regime using governed inputs.
- Regimes are based on realised volatility and drawdown of the strategy.
- Output is used to condition signals, risk, and turnover.
...
- ISO-8601 dates.
- Provenance via regime_run_date.
"""
...
def load_returns() -> pd.DataFrame:
    ...
    if missing:
        msg = f"Returns panel missing columns: {sorted(missing)}"
        logger.error(msg)
        raise ValueError(msg)

    df["date"] = pd.to_datetime(df["date"], errors="coerce", utc=True)
    df = df.dropna(subset=["date", "ticker", "daily_return"])
    df["daily_return"] = pd.to_numeric(df["daily_return"], errors="coerce").fillna(0.0)

    # Aggregate to portfolio-level daily return (equal-weighted cross-section)
    daily = (
...
def load_backtest() -> pd.DataFrame:
    ...
    missing = required - set(df.columns)
    if missing:
        msg = f"Backtest PnL missing columns: {sorted(missing)}"
        logger.error(msg)
        raise ValueError(msg)

    df["date"] = pd.to_datetime(df["date"], errors="coerce", utc=True)
    df = df.dropna(subset=["date", "pnl", "cum_pnl"])
...
def compute_regime_features(
    ...
    # Rolling drawdown over DRAWDOWN_WINDOW
...
def classify_regime(row: pd.Series) -> str:
    ...
    deep_dd = dd < -0.05 * abs(row["cum_pnl"] + 1e-12)  # relative drawdown heuristic

    if high_vol and deep_dd:
        return "crisis"
    if high_vol and not deep_dd:
        return "high_vol"
...
def main() -> None:
    logger.info("Starting regime_classifier_quant_v1 run (v1.0).")
