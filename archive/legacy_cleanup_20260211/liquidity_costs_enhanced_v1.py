...
Output:
    C:/Quant/data/analytics/attribution_outputs_v1/liquidity_costs_enhanced.csv
"""
...
OUT_DIR = DATA_DIR / "attribution_outputs_v1"
...
def load_positions() -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Missing required columns in positions file: {sorted(missing)}")
...
def compute_liquidity_costs(positions: pd.DataFrame) -> pd.DataFrame:
    ...
    df["participation"] = (df["turnover_raw"] / df["adv_20"]).clip(0.0, 1.0)
...
# Save output
# ---------------------------------------------------------------------
def save_output(per_ticker: pd.DataFrame, daily: pd.DataFrame, run_timestamp: str) -> None:
    ...
    common_cols = sorted(set(per_ticker_out.columns) | set(daily_out.columns))
