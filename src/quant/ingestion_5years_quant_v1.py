#!/usr/bin/env python3
# Quant v1.0 — 5-Year Ingestion (Fetcher Only, Container-Safe)
...
def reindex_and_fill(df: pd.DataFrame, cal: pd.DatetimeIndex, t: str) -> Optional[pd.DataFrame]:
    ...
    if non_nan == 0:
        log.info(f"[ingestion_5years_quant_v1] {t}: All OHLCV values NaN after reindex/fill — marking ticker as FAILED")
        return None
...
def run(api_key: Optional[str] = None) -> pd.DataFrame:
    log.info("[ingestion_5years_quant_v1] === Starting 5-year ingestion (Fetcher Only) ===")

    ticker_ref_path = os.getenv(
        "TICKER_REFERENCE_PATH",
        "/app/config/ticker_reference.csv",
    )
