...
#           - date
#           - ticker
#           - <factor columns> (e.g. low_vol_252, volume_zscore, sector_*)
#
...
def determine_factor_columns(exposures: pd.DataFrame, factor_returns: pd.DataFrame) -> list[str]:
    ...
    non_factor = {"date", "ticker", "factor_returns_run_date", "factor_run_date"}
...
def compute_raw_contributions(
    ...
    # Merge factor returns on date
    df = df.merge(factor_returns[["date"] + factor_cols], on="date", how="inner", suffixes=("", "_ret"))

    records = []

    for factor in factor_cols:
        exp_col = factor
        ret_col = factor  # after merge, factor_returns columns share same names

        if exp_col not in df.columns or ret_col not in df.columns:
            continue
