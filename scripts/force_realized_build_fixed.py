# C:\Quant\scripts\force_realized_build_fixed.py
import os
import pandas as pd
ROOT = r"C:\Quant"
...
if date_col is None or ticker_col is None or price_col is None:
    raise SystemExit("missing required columns: date, ticker, or price")
...
for t, g in df.groupby("ticker"):
    ...
    # resample weekly anchored on Monday; take first and last non-null values
    first = g["close"].resample("W-MON").first()
    last = g["close"].resample("W-MON").last()
    w = pd.DataFrame({"close_start": first, "close_end": last})
    w = w.dropna(how="any")
    if w.empty:
        continue
    w = w.reset_index()                 # index -> column named 'date'
    w = w.rename(columns={"date": "week"})
