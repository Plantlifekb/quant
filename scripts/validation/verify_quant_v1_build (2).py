...
REQUIRED_FILES = {
    ...
    "weekly_picks": DATA_ANALYTICS / "quant_weekly_picks_quant_v1.parquet",
...
def verify_optimiser(opt: pd.DataFrame, securities_master: pd.DataFrame) -> None:
    ...
    required_cols = ["date", "ticker", "weight", "portfolio_type"]
    ...
    opt_lo = opt[opt["portfolio_type"] == "long_only"].copy()
    if opt_lo.empty:
        fail("No long-only portfolio found in optimiser output")
    opt_lo["date"] = pd.to_datetime(opt_lo["date"])
    latest_opt_date = opt_lo["date"].max()
    latest_group = opt_lo[opt_lo["date"] == latest_opt_date]
    w_sum = latest_group["weight"].sum()
    if not 0.98 <= w_sum <= 1.02:
        fail(f"Long-only weights do not sum to ~1.0 on {latest_opt_date.date()} (got {w_sum})")
    if (opt_lo["weight"] < 0).any():
        fail("Negative weights found in long-only portfolio")
    ok("Long-only weight sanity checks passed")
    if "ticker" not in securities_master.columns:
        fail("securities_master.parquet missing 'ticker' column")
...
def verify_weekly_picks(picks: pd.DataFrame, opt: pd.DataFrame) -> None:
    print("\n--- 5. Verifying weekly picks layer contracts ---\n")
    required_cols = ["as_of_date", "rank", "ticker", "company_name", "sector", "weight"]
    missing = [c for c in required_cols if c not in picks.columns]
    if missing:
        fail(f"quant_weekly_picks_quant_v1.parquet missing required columns: {missing}")
    if picks["company_name"].isna().any():
        fail("Weekly picks contain missing company_name values")
    if picks["sector"].isna().any():
        fail("Weekly picks contain missing sector values")
    ok("Weekly picks metadata completeness satisfied")
    latest_opt_date = latest_date(opt, "date")
    latest_picks_date = latest_date(picks, "as_of_date")
    if latest_opt_date != latest_picks_date:
        fail(
            f"Weekly picks as_of_date {latest_picks_date} does not match optimiser latest date {latest_opt_date}"
        )
    picks["as_of_date"] = pd.to_datetime(picks["as_of_date"]).dt.date
    for as_of, group in picks.groupby("as_of_date"):
        group_sorted = group.sort_values("rank")
        expected_ranks = list(range(1, len(group_sorted) + 1))
        actual_ranks = list(group_sorted["rank"])
        if actual_ranks != expected_ranks:
            fail(f"Ranks for as_of_date {as_of} are not strictly 1..N")
        weights = list(group_sorted["weight"])
        if weights != sorted(weights, reverse=True):
            fail(f"Weights for as_of_date {as_of} are not sorted descending by rank")
    ok("Weekly picks rank and weight ordering invariants satisfied")
    ok("Weekly picks layer checks passed")
...
def verify_reporting(
    ...
    print("\n--- 6. Verifying reporting layer contracts ---\n")
...
def verify_date_alignment(
    ...
    ok("All pipeline stages share the same latest date")

def verify_dashboard_legacy() -> None:
    print("\n--- 8. Verifying dashboard has no legacy references ---\n")
    LEGACY_PATTERNS = [
...
def main() -> None:
    ...
    try:
        ...
        picks = pd.read_parquet(REQUIRED_FILES["weekly_picks"])
    ...
    verify_weekly_picks(picks, opt)
    verify_reporting(portfolio_performance, factor_exposure, turnover, opt)
    verify_date_alignment(prices, exp, opt, picks)
